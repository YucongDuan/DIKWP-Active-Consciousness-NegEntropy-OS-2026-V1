...
Open `index.html` in a modern browser. No server, network, model API or account is required.

Optional CLI:

```bash
python tools/run_active_consciousness_demo.py examples/sample_agent_state.json --out outputs
```

## What is intentionally unbounded

- Memory fabric schemas do not set fixed `maxItems` limits.
- Intent morphogenesis schemas do not set fixed branch limits.
- Consciousness migration research schemas allow state, memory, self-model, intent and value continuity bundles.
...
- No self-replication or shutdown resistance.
- No financial, medical, traffic, legal, public-safety or high-impact autonomous actions.
- Human Gate is mandatory for C3+ tickets.
